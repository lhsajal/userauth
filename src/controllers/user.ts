...
import { inviteUserValidation, normalSignupValidation, passwordReset, normalLoginValidation } from '../SchemaValidation/inviteUser';
import { emailInvite, createNewUser, passwordResetEmail } from '../services/userService';
...
export const inviteUser: RequestHandler = async (req, res, next) => {
    ...
    try {
        ...
        if (!user) {
            ...
            const prm1 = await emailInvite(email);
...
export const passwordResetEmailSendController: RequestHandler = async (req, res, next) => {
    ...
    try {
        ...
        //validating the input first
        const validationResult = passwordReset.validate(req.body);
        if (validationResult.error) {
...
export const normalLoginController: RequestHandler = async (req, res, next) => {
    ...
    try {
        ...
        //validating the input first
        const validationResult = normalLoginValidation.validate(req.body);
        if (validationResult.error) {
